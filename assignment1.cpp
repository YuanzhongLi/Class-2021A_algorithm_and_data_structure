...
void measure(vector<int> &array) {
  int N = array.size();
  int target = array[N/2];

  int liner_start, liner_end, binary_search_start, binary_search_end;
  ...
  cout << N << " " << liner_end - liner_start << ", " << binary_search_end - binary_search_start << endl;
};
...
signed main() {
  ...
  vector<int> sizes = {100000, 1000000, 10000000, 1000000000};

  cout << "実行結果" << endl;
  cout << endl;
  cout << "n, 線型探索(ms), 2分探索(ms)" << endl;
  for (int size: sizes) {
    vector<int> array(size);
    rep(i,0,size) array[i] = i;
    measure(array);
  }
